...
#define DEVICE_NAME                      "BMD300Eval"                             /**< Name of device. Will be included in the advertising data. */
#define MANUFACTURER_NAME                "Rigado"                                   /**< Manufacturer. Will be passed to Device Information Service. */
...
static ble_bmdeval_t                     m_bmdeval;
...
{
    uint32_t                           err_code;
    ble_bmdeval_init_t                 bmdeval_init;
    ble_dis_init_t                     dis_init;

    // Initialize XXX Service.
    memset(&bmdeval_init, 0, sizeof(bmdeval_init));

    ble_bmdeval_handler_init(&bmdeval_init, &m_bmdeval);
    
    err_code = ble_bmdeval_init(&m_bmdeval, &bmdeval_init);
    APP_ERROR_CHECK(err_code);

    // Initialize Device Information Service    
    memset(&dis_init, 0, sizeof(dis_init));
    
    ble_srv_ascii_to_utf8(&dis_init.manufact_name_str, "Rigado");
    ...
    ble_srv_ascii_to_utf8(&dis_init.hw_rev_str, HARDWARE_VERSION_STRING);
    ble_srv_ascii_to_utf8(&dis_init.model_num_str, "BMD-300 Eval Shield" );
...
static void ble_stack_init(void)
{
    ...
    //TODO: If your board does not have an external crystal, define
    //USE_INTERNAL_RC.
#ifndef USE_INTERNAL_RC
...
void bsp_event_handler(bsp_event_t event)
{
    ...
    bmd_button_event_handler(event);
}
...
static void advertising_init(void)
{
    ...
    adv_uuids.uuid = BMDEVAL_UUID_SERVICE;
    adv_uuids.type = m_bmdeval.uuid_type;
...
int main(void)
{
    ...
    simple_uart_config(RTS_PIN_NUMBER, TX_PIN_NUMBER, CTS_PIN_NUMBER, 
        RX_PIN_NUMBER, false, UART_BAUDRATE_BAUDRATE_Baud115200, 0);
    run_test_suite();
    while(1);
#else
    uint32_t err_code;
    ...
    advertising_init();
    conn_params_init();
    bmd300_demo_shield_init(&m_bmdeval);
